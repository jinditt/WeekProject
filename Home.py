...
st.subheader('대한민국 **역사 유적지** 방문자를 위한 가이드', divider='rainbow')
...
with col1:
   ...
   st.image('data/그림1.png')

with col2:
   centered_text("<b>리뷰를 한눈에<b>")
   #st.write("리뷰를 한눈에")
   st.image('data/그림2.png')
...
st.write('---')
st.subheader('바로가기')
st.page_link('pages/01_전국별.py', label='***전국의 유적지 정보***', icon='🗺️')
st.page_link('pages/02_지역별.py', label='***각 지역 유적지 정보***', icon='🏛️')